...
## 2026-07-16: 기록 탭 리스트 아이템 높이 축소

- **사용자 지시**: "기록 리스트의 아이템 항목의 각각이 여백이 있는 아이템처럼 되다 보니 몇개 리스트에 나오면 다 차보인다. 좀 높이를 줄여서 보이게 해서 여러 줄이 나올 수 있게 하자" — 화면당 보이는 기록 수가 너무 적어서(5줄 정도) 스크롤을 자주 해야 하는 문제.
- **구현**: `HomeTabs.kt`의 `EventRow`(기록 탭 리스트·현황 탭 날짜별 상세 다이얼로그 공용) 내부 `Row` 패딩을 `padding(12.dp)`(사방 동일)에서 `padding(horizontal = 12.dp, vertical = 6.dp)`로 줄이고, 아이콘을 20dp로 축소, 유형 라벨(`titleSmall`→`bodyMedium`)도 살짝 축소. `RecordsTab`의 `LazyColumn` 아이템 간 간격도 `spacedBy(8.dp)`→`spacedBy(4.dp)`로 줄임.
- **실기기 검증**: 재설치 후 기록 탭 확인 — 이전엔 화면에 5줄 보이던 게 7줄로 늘어남. 텍스트 겹침/잘림 없음, 클릭 영역도 정상 동작.
- **How to apply**: `EventRow`는 기록 탭과 `DayDetailDialog` 양쪽에서 공유되는 컴포저블이라, 이번 축소가 두 곳 모두에 자동 반영됨 — 리스트 밀도 관련 요청이 오면 이 컴포넌트 하나만 고치면 된다는 점 기억.
- **추가 축소(같은 날 후속 요청)**: "상하 여백을 더 줄여줘"라는 후속 요청으로 `EventRow`의 `vertical = 6.dp`→`2.dp`, `LazyColumn`의 `spacedBy(4.dp)`→`spacedBy(2.dp)`로 한 번 더 축소. 실기기 확인 결과 텍스트 겹침 없이 더 조밀하게 표시됨 — 이 정도(수직 패딩 2dp)가 사실상 하한에 가까우므로, 다음에 또 줄여달라는 요청이 오면 패딩보다 폰트 크기(`bodySmall`→더 작은 스타일) 쪽을 먼저 검토할 것.

## 2026-07-16: 스캔 캐시 미스로 인한 가짜 1분 자리비움 — 실연결도 OR 조건으로 인정

- **증상**: "자리비움으 10:01~02까지 떳네. 실제는 회사에서 계속 있었는데." [[project_commute_wifi_scan_detection]]에서 이미 예견했던 실패 유형 그대로 실전에서 발생 — `isCompanyWifiNearby`가 `WifiManager.scanResults` 캐시만 보는데, 이 캐시가 폴링 틱 한 번 갱신을 놓치거나 순간적으로 비면 "회사 와이파이 안 보임"으로 오판해 자리비움 유예가 시작되고, 바로 다음 틱에 다시 잡히면서 1분짜리 자리비움(자리비움 시작~복귀가 같은 폴링 간격)이 기록됨.
- **수정**: `WifiUtils.kt`의 `isCompanyWifiNearby`에 `wifiManager.connectionInfo.cleanSsid() == companySsid`를 먼저 확인하는 OR 조건 추가 — 실제로 연결 중이면 스캔 캐시 상태와 무관하게 무조건 "근처"로 인정. "범위 안에만 있어도 인정"하는 기존 스캔 기반 취지는 그대로 두고(대체가 아니라 보강), 스캔 캐시가 흔들려도 최소한 "진짜 연결 중"인 동안은 절대 놓치지 않게 됨.
- **실기기 검증**: 재설치 후 상태 카드가 "근무중 · 점심시간"으로 정상 표시, 크래시 없음. (스캔 캐시가 실제로 미스나는 순간을 인위적으로 재현하긴 어려워서, 이번 수정 자체가 그 순간을 막는지 라이브로 재현 검증은 못 함 — 코드 논리상 확실한 보강이라 판단.)
- **잔여 데이터**: 이미 기록된 10:01~10:02 가짜 자리비움 건은 그대로 남아있음(1분이라 근무시간 계산엔 영향 없음) — 사용자가 기록 탭에서 직접 삭제하거나 요청하면 대신 지워주기로 함.
- **How to apply**: [[project_commute_wifi_scan_detection]]에 적어둔 "스캔 기반 감지가 불안정하면 연결 기반을 OR로 추가"라는 폴백 계획이 그대로 맞아떨어진 케이스 — 미리 실패 모드를 예측해서 메모에 남겨두는 게 실제로 그 버그가 터졌을 때 원인 진단 속도를 크게 줄여줌.

## 2026-07-16: 출퇴근 알림이 계속 쌓여서 런처 뱃지가 원인 불명으로 늘어나는 버그 수정

- **증상**: "앱 아이콘에 뱃지에 2라고 표시된다 앱에 들어가면 뱃지에 연관된 알림이나 내용이 없어 보이는데.."
- **원인**: `Notifications.kt`의 `showEventNotification()`이 `private var eventNotificationId = 1000`을 매 호출마다 `eventNotificationId++`로 증가시켜 알림을 올림 — 출근/퇴근/자리비움 종료마다 서로 다른 새 ID를 써서, 이전 알림을 대체하지 않고 계속 별개로 쌓이기만 함. 사용자가 하나하나 직접 스와이프해서 안 지우면 알림함에 무기한 누적됨.
- **조사**: `dumpsys notification --noredact`로 실제 활성 알림을 확인한 결과, 이 순간 진짜로 떠 있는 알림은 "출퇴근 감지 중"(포그라운드 서비스, `commute_monitor` 채널, `IMPORTANCE_MIN`) 1개뿐이었고, 알림함(펼친 화면)을 직접 봐도 Commute 알림이 안 보였음(최소 중요도라 알림함 UI에 아예 안 그려지는 것으로 보임) — 뱃지 숫자 "2"에 대응하는 실제 알림 2개를 못 찾음. 삼성 원UI 런처의 뱃지 캐시가 과거 자동취소된 알림 개수를 못 따라가고 스터일 값을 보여주는 것으로 추정(단, 확증은 못 함).
- **수정**: `eventNotificationId`(증가하는 var)를 `EVENT_NOTIFICATION_ID`(고정 상수)로 변경 — 이제 새 기록 알림이 이전 걸 항상 대체(최근 것 하나만 유지). 알림이 무한 누적되는 구조적 원인 자체를 제거.
- **실기기 검증**: 재빌드·재설치 후 크래시 없음. 뱃지가 "지금 당장" 정상값으로 바뀌는지는 다음 실제 출퇴근 이벤트가 발생해야 확인 가능(라이브 재현 못 함).
- **How to apply**: 알림을 "이벤트 로그"처럼 여러 개 남기고 싶은 게 아니라면, `notify()`에 쓰는 ID는 절대 무한정 증가시키지 말고 고정 ID(또는 명확한 유한 집합)를 재사용할 것 — 그래야 알림 개수가 저절로 상한선을 가짐. 이 프로젝트에서 `MONITOR_NOTIFICATION_ID`처럼 상수로 고정해둔 다른 알림들과 패턴을 통일.
